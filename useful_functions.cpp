...
{
    (Onegin->beginning_of_lines)[0] = Onegin->original;//Говорю что нет пустых строк
    int j = 1;
    for (int i = 0; i < Onegin->text_size; i++)
    {
        ...
        */
        ...
        {
            (Onegin->beginning_of_lines)[j] = (Onegin->original + (++i));
            j++;
        }
    }
    return (j-2);
...
void read_in_array (char *arr, struct text_param *Onegin)
{
    ...
    {
        int count = 0;
        for (; Onegin->beginning_of_lines[j][count] != '\n'; count++, i++ )
        {
            arr[i] = Onegin->beginning_of_lines[j][count];
        }
        printf ("Сейчас в рид аррэй такая интеарция: %d и такая строка: %p  %d\n", j, Onegin->beginning_of_lines[j], i);
        arr[i] = Onegin->beginning_of_lines[j][count];
        j++;
    }
}
